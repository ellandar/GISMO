...
public interface SubProjectRepository extends JpaRepository<SubProject, Integer> {
    ...
    default List<SubProject> complete(String query, int maxResults) {
        SubProject probe = new SubProject();
        probe.setName(query);

        ExampleMatcher matcher = ExampleMatcher.matching() //
                .withMatcher(SubProject_.name.getName(), match -> match.ignoreCase().startsWith());

        Page<SubProject> page = findAll(Example.of(probe, matcher), new PageRequest(0, maxResults));
