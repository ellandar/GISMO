...
public interface ProjectGroupRepository extends JpaRepository<ProjectGroup, Integer> {
    ...
    default List<ProjectGroup> complete(String query, int maxResults) {
        ProjectGroup probe = new ProjectGroup();
        probe.setName(query);

        ExampleMatcher matcher = ExampleMatcher.matching() //
                .withMatcher(ProjectGroup_.name.getName(), match -> match.ignoreCase().startsWith());

        Page<ProjectGroup> page = findAll(Example.of(probe, matcher), new PageRequest(0, maxResults));
