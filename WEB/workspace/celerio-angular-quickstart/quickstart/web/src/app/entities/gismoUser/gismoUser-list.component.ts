...
import { GismoUserDetailComponent } from './gismoUser-detail.component';
import { GismoUserService } from './gismoUser.service';

@Component({
    moduleId: module.id,
	templateUrl: 'gismoUser-list.component.html',
	selector: 'gismoUser-list'
})
export class GismoUserListComponent {

    @Input() header = "GismoUsers...";
    ...
    gismoUserToDelete : GismoUser;

    // basic search criterias (visible if not in 'sub' mode)
    example : GismoUser = new GismoUser();

    // list is paginated
    currentPage : PageResponse<GismoUser> = new PageResponse<GismoUser>(0,0,[]);


    constructor(private router : Router,
        private gismoUserService : GismoUserService,
    ...
    loadPage(event : LazyLoadEvent) {
        this.gismoUserService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/gismoUser', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/gismoUser', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let gismoUserToDelete : GismoUser = <GismoUser> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(gismoUserToDelete);
            }
        });
    }

    private delete(gismoUserToDelete : GismoUser) {
        let id =  gismoUserToDelete.id;

        this.gismoUserService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(gismoUserToDelete);
