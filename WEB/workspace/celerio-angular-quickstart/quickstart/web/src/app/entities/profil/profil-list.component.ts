...
export class ProfilListComponent {
    ...
    profilToDelete : Profil;

    // basic search criterias (visible if not in 'sub' mode)
    example : Profil = new Profil();
    ...
    loadPage(event : LazyLoadEvent) {
        this.profilService.getPage(this.example, event).
            subscribe(
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/profil', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let profilToDelete : Profil = <Profil> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(profilToDelete);
            }
        });
    }

    private delete(profilToDelete : Profil) {
        let id =  profilToDelete.id;

        this.profilService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(profilToDelete);
