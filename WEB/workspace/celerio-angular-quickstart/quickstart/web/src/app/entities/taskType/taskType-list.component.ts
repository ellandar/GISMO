...
import { TaskTypeDetailComponent } from './taskType-detail.component';
...
export class TaskTypeListComponent {
    ...
    example : TaskType = new TaskType();

    // list is paginated
    currentPage : PageResponse<TaskType> = new PageResponse<TaskType>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.taskTypeService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/taskType', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/taskType', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let taskTypeToDelete : TaskType = <TaskType> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(taskTypeToDelete);
            }
        });
    }

    private delete(taskTypeToDelete : TaskType) {
        let id =  taskTypeToDelete.id;

        this.taskTypeService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(taskTypeToDelete);
