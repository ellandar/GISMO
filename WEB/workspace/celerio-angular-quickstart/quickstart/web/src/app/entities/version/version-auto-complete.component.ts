...
export const VERSION_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => VersionCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-version pTemplate="item">
                <version-line [version]="version"></version-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'version-auto-complete',
    providers: [VERSION_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class VersionCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Version = null;

    public suggestions : Version[] = [];

    //Placeholders for the callbacks
    private _onTouchedCallback: () => void = () => {};
    private _onChangeCallback: (_:any) => void = () => {};

    constructor(private versionService : VersionService, private messageService : MessageService) {
    }
    ...
    complete(event:any) {
        this.versionService.complete(event.query).
            subscribe(
