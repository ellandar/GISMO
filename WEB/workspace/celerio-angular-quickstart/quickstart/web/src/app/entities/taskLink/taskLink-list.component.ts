...
import { TaskLinkDetailComponent } from './taskLink-detail.component';
import { TaskLinkService } from './taskLink.service';

@Component({
    moduleId: module.id,
	templateUrl: 'taskLink-list.component.html',
	selector: 'taskLink-list'
})
export class TaskLinkListComponent {

    @Input() header = "TaskLinks...";
    ...
    taskLinkToDelete : TaskLink;

    // basic search criterias (visible if not in 'sub' mode)
    example : TaskLink = new TaskLink();

    // list is paginated
    currentPage : PageResponse<TaskLink> = new PageResponse<TaskLink>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.taskLinkService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        let id = event.data.id.childTaskId + '_' + 
event.data.id.parentTaskId;
        this.router.navigate(['/taskLink', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/taskLink', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let taskLinkToDelete : TaskLink = <TaskLink> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(taskLinkToDelete);
            }
        });
    }

    private delete(taskLinkToDelete : TaskLink) {
        let id = taskLinkToDelete.id.childTaskId + '_' + taskLinkToDelete.id.parentTaskId;

        this.taskLinkService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(taskLinkToDelete);
